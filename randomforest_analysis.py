...
class FeatureImportance():
    ...
    Parameters
    ----------
    methods : {"gain", "dfc", "perm", "drop"}, default=None
    ...
    use_oob : bool, default=True
        If True, the algorithm passes out-of-bag (OOB) samples
        through `estimator`, otherwise `X_train` is used as
        a validation set.
...
def Calibrate(y_true, y_proba, equal_width=True, bins=10):
    ...
    # Calculate Beta
    p, t = info[:,[3]], info[:,[5]]
    beta = np.linalg.inv(p.T.dot(p)).dot(p.T).dot(t)
